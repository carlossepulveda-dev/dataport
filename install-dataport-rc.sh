...
if ! RELEASE_JSON="$(
  ...
  echo "GitHub may be unavailable or the unauthenticated API rate limit may have been reached." >&2
  exit 1
fi
...
RELEASE_JSON="$(
  jq '
    map(select(
      .draft == false and
      .prerelease == true and
      (.tag_name | test("^v?[0-9]+\\.[0-9]+\\.[0-9]+-rc\\.[0-9]+$"))
    )) |
...
if ! [[ "$VERSION" =~ $VERSION_PATTERN ]] || [ "$PRERELEASE" != "true" ]; then
  echo "No valid RC prerelease was found in the newest 100 releases." >&2
  exit 1
fi
...
# Older RC releases used the base package version without the RC suffix.
if [ -z "$DOWNLOAD_URL" ]; then
  LEGACY_PACKAGE_NAME="dataport_${VERSION%%-rc.*}_all.deb"
  DOWNLOAD_URL="$(
    jq -r --arg name "$LEGACY_PACKAGE_NAME" '
      .assets[]? | select(.name == $name) | .browser_download_url
    ' <<<"$RELEASE_JSON" | head -n 1
  )"
  MATCHED_ASSET="$(jq -r --arg url "$DOWNLOAD_URL" '.assets[]? | select(.browser_download_url == $url) | .name' <<<"$RELEASE_JSON" | head -n 1)"
  if [ -n "$DOWNLOAD_URL" ]; then
    echo "Using legacy RC asset name: $MATCHED_ASSET" >&2
  fi
fi

if [ -z "$DOWNLOAD_URL" ]; then
  echo "Release $TAG does not contain $PACKAGE_NAME or its legacy RC asset name." >&2
